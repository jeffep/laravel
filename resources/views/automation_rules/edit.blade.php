        ...
        <div class="mb-3">
            ...
            <select name="location" class="form-control" id="location" required>
                @foreach ($locations as $location)
                    <option value="{{ $location }}" {{ $location == $automationRule->location ? 'selected' : '' }}>{{ $location }}</option>
                @endforeach
            </select>
        </div>
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <select name="action_device_id" class="form-control" required>
                @foreach ($devices as $device)
                    <option value="{{ $device->id }}" {{ $device->id == $automationRule->action_device_id ? 'selected' : '' }}>{{ $device->name }}</option>
                @endforeach
            </select>
        </div>
        <div class="mb-3">
